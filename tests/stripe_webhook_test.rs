...
use coterie::{
    auth::SecretCrypto,
    domain::{
        BillingMode, CreateMemberRequest, MembershipType, Payment, PaymentMethod, PaymentStatus,
        PaymentType,
    },
    email::LogSender,
...
async fn charge_refunded_echo_for_already_refunded_row_is_noop() {
    ...
    insert_pending_payment(
        ...
        Payment {
            ...
            payment_method: PaymentMethod::Stripe,
            stripe_payment_id: Some(pi_id.to_string()),
            description: "Dues".to_string(),
            payment_type: PaymentType::Membership,
            donation_campaign_id: None,
            donor_name: None,
...
async fn public_donation_checkout_completion_marks_payment_completed() {
    ...
    insert_pending_payment(
        ...
        Payment {
            ...
            status: PaymentStatus::Pending,
            payment_method: PaymentMethod::Stripe,
            stripe_payment_id: Some(session_id.to_string()),
            description: "Donation — Anonymous".to_string(),
            payment_type: PaymentType::Donation,
            donation_campaign_id: None,
            donor_name: Some("Anonymous Donor".to_string()),
...
async fn webhook_handlers_do_not_call_gateway_unnecessarily() {
    ...
    insert_pending_payment(
        ...
        Payment {
            ...
            payment_type: PaymentType::Donation,
            donation_campaign_id: None,
            donor_name: Some("D".to_string()),
            donor_email: Some("d@example.com".to_string()),
            paid_at: Some(Utc::now()),
            created_at: Utc::now(),
            updated_at: Utc::now(),
        },
