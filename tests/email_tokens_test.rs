...
async fn consume_redeems_exactly_once() {
    ...
    assert!(
        second.is_none(),
        "second consume of same token must return None"
    );
}
...
async fn consume_rejects_expired_token() {
    ...
    let consumed_at: Option<chrono::NaiveDateTime> =
        sqlx::query_scalar("SELECT consumed_at FROM password_reset_tokens WHERE token_hash = ?")
            .bind(&token_hash)
            .fetch_one(&pool)
            .await
    ...
    assert!(
        consumed_at.is_none(),
        "expired token's consumed_at must remain NULL"
    );
}
...
async fn invalidate_for_member_marks_outstanding_consumed() {
    ...
    assert!(
        consume_password_reset_token(&pool, &a.token)
            .await
            .unwrap()
            .is_none(),
    ...
    assert!(
        consume_password_reset_token(&pool, &b.token)
            .await
            .unwrap()
            .is_none(),
...
async fn cleanup_expired_deletes_only_expired_rows() {
    ...
    assert_eq!(
        deleted, 1,
        "cleanup_expired must delete exactly the expired row"
    );
    ...
    assert!(
        consumed.is_some(),
        "unexpired token must still redeem after cleanup"
    );
}
...
async fn created_token_is_sha256_of_plaintext() {
    ...
    let stored_hash: String =
        sqlx::query_scalar("SELECT token_hash FROM password_reset_tokens WHERE member_id = ?")
            .bind(member_id.to_string())
            .fetch_one(&pool)
            .await
