...
fn admin_alerts(events: &Arc<Mutex<Vec<IntegrationEvent>>>) -> Vec<(String, String)> {
    events
        .lock()
        .unwrap()
        .iter()
        .filter_map(|e| match e {
            IntegrationEvent::AdminAlert { subject, body } => Some((subject.clone(), body.clone())),
            _ => None,
        })
        .collect()
}
...
async fn seed_default_card(saved_card_repo: &Arc<SqliteSavedCardRepository>, member_id: Uuid) {
    use chrono::Datelike;
    let now = Utc::now();
    let card = SavedCard {
        id: Uuid::new_v4(),
...
async fn seed_scheduled_payment(
    ...
    let created = scheduled_repo
        .create(sp)
        .await
        .expect("create scheduled_payment");
    created.id
}
...
async fn terminal_failure_dispatches_admin_alert() {
    ...
    // (2 + 1 >= 3).
    let scheduled_id = seed_scheduled_payment(&h.scheduled_repo, member_id, mt_id, 2).await;
    ...
    assert!(
        terminal
            .1
            .contains(&format!("/portal/admin/members/{}", member_id)),
