...
pub trait StripeGateway: Send + Sync {
    ...
    async fn create_setup_intent(&self, input: CreateSetupIntentInput)
...
impl StripeGateway for RealStripeGateway {
    ...
    async fn create_payment_intent(
        ...
        let cid: CustomerId = input
            .customer_id
            .parse()
            .map_err(|_| AppError::Internal("Invalid customer ID".to_string()))?;
        let pmid: PaymentMethodId = input
            .payment_method_id
            .parse()
            .map_err(|_| AppError::Internal("Invalid payment method ID".to_string()))?;
        ...
        let idempotent_client = self
            .client
            .clone()
            .with_strategy(RequestStrategy::Idempotent(input.idempotency_key.clone()));
        let intent = timed(PaymentIntent::create(&idempotent_client, params))
            .await
            .map_err(|e| match e {
                AppError::External(msg) => {
                    AppError::External(format!("Stripe charge failed: {}", msg))
                }
                other => other,
            })?;
        ...
        Ok(match intent.status {
            ...
            other => PaymentIntentResult::Other {
                id,
                status: format!("{:?}", other),
            },
        })
    ...
    async fn retrieve_payment_method(
        ...
        let (brand, last4, exp_month, exp_year) = pm
            .card
            .as_ref()
            .map(|c| {
                (
    ...
    async fn create_refund(&self, input: CreateRefundInput) -> Result<RefundOutput> {
        ...
        let idempotent_client = self
            ...
            .clone()
            .with_strategy(RequestStrategy::Idempotent(input.idempotency_key.clone()));
        let refund = timed(Refund::create(&idempotent_client, params))
            .await
            .map_err(|e| match e {
                AppError::External(msg) => {
                    AppError::External(format!("Stripe refund failed: {}", msg))
                }
                other => other,
            })?;
        Ok(RefundOutput {
    ...
    async fn retrieve_invoice(&self, invoice_id: &str) -> Result<RetrievedInvoice> {
        let inv_id: InvoiceId = invoice_id
            .parse()
            .map_err(|_| AppError::BadRequest(format!("Invalid invoice ID: {}", invoice_id)))?;
