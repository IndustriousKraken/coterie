...
pub fn create_web_routes(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            "/forgot-password",
            get(templates::reset::forgot_password_page),
        )
        .route(
            "/forgot-password",
            post(templates::reset::forgot_password_handler),
        )
        .route(
            "/reset-password",
            get(templates::reset::reset_password_page),
        )
        .route(
            "/reset-password",
            post(templates::reset::reset_password_handler),
        )
        // Portal routes
