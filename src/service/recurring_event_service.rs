    ...
    pub async fn create_series_with_initial_materialization(
        ...
        for (idx, start) in occurrence_times.iter().enumerate() {
            ...
            let mut occurrence = Event {
                ...
                member_price_cents: template.member_price_cents,
                guest_price_cents: template.guest_price_cents,
                guest_registration_enabled: template.guest_registration_enabled,
