...
impl MemberService {
    ...
    pub async fn bulk_import(
        ...
        for (idx, row) in rows.into_iter().enumerate() {
            ...
            let membership_type = match active_types
                .iter()
                .find(|t| t.slug == slug && t.is_active)
            {
                Some(mt) => mt,
                None => {
                    summary.failed += 1;
            ...
            if let Some(discord_id) = row.discord_id.as_ref().map(|s| s.trim()).filter(|s| !s.is_empty()) {
                if let Err(e) = self
                    .member_repo
