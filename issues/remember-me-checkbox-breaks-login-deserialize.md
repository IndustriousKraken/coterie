...
- [ ] 1.1 In src/web/templates/auth.rs, change LoginRequest.remember_me from Option<bool> to `#[serde(default)] remember_me: Option<String>`, matching the existing HTML-checkbox convention used in src/web/portal/admin/unifi.rs / discord.rs / stripe.rs (present="on" when checked, absent otherwise).
- [ ] 1.2 Update the three read sites in login_handler (the pending_login create call and the two session/cookie duration branches) to treat the checkbox as checked via `credentials.remember_me.is_some()` instead of `.unwrap_or(false)`.
