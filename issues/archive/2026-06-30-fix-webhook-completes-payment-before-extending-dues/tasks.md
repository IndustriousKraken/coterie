...
- [x] 1.1 In `src/payments/webhook_dispatcher/checkout.rs::handle_successful_payment`,
...
- [x] 1.2 For the **donation** branch, keep the flip as the terminal step:
...
- [x] 1.3 For the **membership** branch with a resolvable slug, call
...
- [x] 1.4 For the **slug-unresolvable** branch, keep current behavior:
...
- [x] 1.5 Confirm that a transient error returned by
...
- [x] 2.1 In `src/payments/webhook_dispatcher/payment_intent.rs::handle_payment_intent_succeeded`,
  keep the existing metadata/member/amount cross-checks (lines ~53-125)
  ahead of any mutation — they must stay before the flip.
- [x] 2.2 For the **non-membership** (donation/other) case, keep the flip
  terminal: `complete_pending_payment`; if `false`, return `Ok(())`;
  otherwise return `Ok(())` (no dues work).
- [x] 2.3 For the **membership** case, resolve `member_id`, the member,
...
- [x] 2.4 Preserve the existing early-return no-ops (missing
...
- [x] 3.1 Add a test to `tests/stripe_webhook_test.rs` (using the
...
- [x] 3.2 Extend that test to prove recovery: create the membership type
...
- [x] 3.3 Add the analogous test for
...
- [x] 3.4 Keep/confirm an idempotency assertion: dispatching a
