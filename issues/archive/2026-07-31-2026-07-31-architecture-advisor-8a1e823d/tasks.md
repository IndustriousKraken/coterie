...
- [x] 1.1 Convert `src/api/handlers/public.rs` into a module directory
...
- [x] 1.2 Create `public/signup.rs` holding the signup funnel (current
  ...
  `SignupResponse` DTOs.
- [x] 1.3 Create `public/feeds.rs` holding the feed serialization (current
  1041–1180): `escape_cdata`, `generate_rss_feed`, `escape_ical_text`,
  `generate_ical_feed`, and the `rss_feed` / `calendar_feed` handlers
  (969–1020).
- [x] 1.4 Create `public/donate.rs` holding the donation path (current
  1181–1362): `PublicDonateRequest`, `PublicDonateResponse`, and `donate`.
- [x] 1.5 Create `public/register.rs` holding guest registration and class
  ...
  verbatim.
- [x] 1.6 Leave in `public/mod.rs`: the read projections `list_events`,
...
- [x] 1.7 In `public/mod.rs` declare the four submodules and re-export
  ...
  compiles unchanged.
- [x] 1.8 Keep the `#[cfg(test)] mod announcement_markdown_tests` block
...
- [x] 1.9 Reconcile `use` imports per submodule: copy the original file's
  ...
  pull neither.
- [x] 1.10 Confirm `cargo test --features test-utils` still produces the
...
- [x] 2.1 In `src/web/portal/admin/events/single.rs`, add a private
  ...
  `remove_image`.
- [x] 2.2 Add `async fn parse(multipart: &mut Multipart) -> Result<EventForm,
...
- [x] 2.3 Preserve each field's exact defaults and parse semantics:
...
- [x] 2.4 Preserve the enum fallbacks exactly: an unrecognized
  `event_type` → `EventType::Meeting`, an unrecognized `visibility` →
  `EventVisibility::MembersOnly`.
- [x] 2.5 Preserve the datetime conversion exactly:
...
- [x] 2.6 Unify the invalid-start-time response: `admin_update_event`
...
- [x] 2.7 Rewrite `admin_create_event` (437–603) and `admin_update_event`
...
- [x] 2.8 Net result is a deletion: the duplicated ~95-line drain loop, the
...
- [x] 3.1 Add the generalized builders to `tests/common/mod.rs` as `pub`
...
- [x] 3.2 Delete the local copies from `tests/paid_events_test.rs`,
...
- [x] 3.3 Keep the thin per-file wrappers that only bind this-test-specific
  ...
  `common`.
- [x] 3.4 Verify with `grep -rn "fn charge\|fn build_charge\|\"object\": \"checkout.session\"" tests/`
  that each builder body has exactly one definition and it lives in
  `tests/common/mod.rs`.
- [x] 3.5 Change no assertion in any of the four binaries; every test that
  passed before still passes.

## 4. Split `install.rs` into `install/inputs.rs` + `install/executor.rs`

- [x] 4.1 Convert `deploy/coterie-provision/src/install.rs` into
...
- [x] 4.2 Move to `install/inputs.rs` the operator interview (current
...
- [x] 4.3 Move to `install/executor.rs` the box mutation (current
...
- [x] 4.4 Leave in `install/mod.rs`: `StripeMode` and its `as_str` /
...
- [x] 4.5 Re-export from `install/mod.rs` everything the crate's public
...
- [x] 4.6 Split the inline `#[cfg(test)] mod tests` (1315–end) to follow
  ...
  assertions.
- [x] 4.7 Preserve exactly, per the `provisioning-wizard` capability: every
...
- [x] 4.8 Keep `deploy/coterie-provision/tests/install_flow.rs` and its
...
- [x] 5.1 `cargo build --features test-utils` — no compiler errors beyond
  the pre-refactor baseline.
- [x] 5.2 `cargo test --features test-utils` — every test that passed
  before still passes; none are added, removed, or have changed assertions.
- [x] 5.3 `cargo test --manifest-path deploy/coterie-provision/Cargo.toml`
  — the `install_flow`, `caddyfile`, `switch_to_live`, and `update_flow`
  suites all still pass, golden snapshots included.
- [x] 5.4 `cargo clippy --features test-utils` and `cargo fmt --check` — no
  new warnings or formatting diffs relative to the baseline.
- [x] 5.5 Spot-check that each split actually relocated code (each named
