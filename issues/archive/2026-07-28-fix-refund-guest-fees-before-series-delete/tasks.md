...
- [x] 1.1 In `src/web/portal/admin/events/single.rs::admin_delete_event`,
...
- [x] 1.2 Replace the `if event.is_paid_for_members()` test at
  ...
  can use it.
- [x] 1.3 Fail closed on a lookup error: if
  ...
  roster.
- [x] 1.4 Update the block comment above the guard
...
- [x] 2.1 Add a test `series_delete_refused_when_a_guest_paid_an_occurrence`
...
- [x] 2.2 Add `series_delete_allowed_when_no_occurrence_was_paid`: same
...
- [x] 2.3 Add `end_series_refused_when_a_member_paid_an_occurrence` to keep
