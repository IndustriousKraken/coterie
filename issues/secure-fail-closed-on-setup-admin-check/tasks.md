...
- [ ] 1.1 In `src/web/templates/setup.rs::check_admin_exists`, change the
...
- [ ] 1.2 In `src/web/templates/setup.rs::setup_handler`, before acquiring
...
- [ ] 2.1 Add a test `setup_refuses_when_admin_check_errors` in
...
- [ ] 2.2 Add a test `setup_refuses_when_admin_already_observed`: build the
...
- [ ] 2.3 Add (or extend) a happy-path test `setup_creates_first_admin` on
