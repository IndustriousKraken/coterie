...
- [ ] 1.1 In `src/web/templates/setup.rs::setup_handler`, replace the
  ...
  `success: true` return.
- [ ] 1.2 Confirm by reading the surrounding code that on this new early
...
- [ ] 2.1 Add a small helper in `setup.rs` (e.g.
  ...
  `check_admin_exists`).
- [ ] 2.2 Call this cleanup on BOTH post-create failure paths before returning
...
- [ ] 2.3 Verify the cleanup is NOT run on the success path and does not touch
  any row other than the just-created `member.id`.

## 3. Regression test: setup yields an Active admin

- [ ] 3.1 Add an integration test (new file
  ...
  `Pending`.
- [ ] 3.2 Assert the same request returns `200` with `success: true` and the
  `/login` redirect, so the test also guards the unchanged happy-path contract.

## 4. Verify

- [ ] 4.1 Run `cargo test` (the existing `setup_redirect_test`,
  `create_admin_test`, and `csrf_coverage_test` suites plus the new test) and
  confirm all pass.
- [ ] 4.2 Run `cargo clippy` on the touched file and confirm no new warnings.
