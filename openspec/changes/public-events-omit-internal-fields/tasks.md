...
- [ ] 1.1 Add a `PublicEvent` serialize struct in
  ...
  `occurrence_index`.
- [ ] 1.2 In `list_events`, keep combining `list_public()` + sanitized
  ...
  path unchanged.
- [ ] 1.3 Register `PublicEvent` in `src/api/docs.rs`; update the `list_events`
  response body schema to `[PublicEvent]`.

## 2. Tests

- [ ] 2.1 Assert the JSON `/public/events` response for a PUBLIC event omits
  `created_by`, `created_at`, `updated_at`, `event_type_id`, `series_id`,
  `occurrence_index`.
- [ ] 2.2 Assert a MEMBERS-ONLY event still has the sanitized
