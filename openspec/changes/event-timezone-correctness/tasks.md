...
- [ ] 1.1 Add an IANA timezone dependency (e.g. `chrono-tz`).
- [ ] 1.2 In `src/service/settings_service.rs`, add an `org.timezone`
  constant (category `organization`, default `UTC`). Validate on write that
  the value parses as an IANA zone; reject unknown names.
- [ ] 1.3 Surface it on the settings page (a dropdown of common zones beats a
  free-text field). Add a `settings.org_timezone() -> Tz` helper that falls
  back to `UTC`.

## 2. Local + zone storage model

- [ ] 2.1 Migration: add a `timezone` TEXT column to `events` (and the series
  ...
  stored time value.
- [ ] 2.2 Change the domain `Event` representation so its time is understood
...
- [ ] 3.1 In `src/web/portal/admin/events/single.rs`, store the form's naive
  input as-is together with the event zone (defaulted from `org.timezone`);
  no conversion on save.
- [ ] 3.2 Pre-fill the edit form and render admin lists/detail from the stored
  wall-clock directly (optionally annotate the zone abbreviation, e.g.
  "7:00 PM EDT").

## 4. Derive UTC at read time (public + iCal)

- [ ] 4.1 In `src/api/handlers/public.rs`, serialize `/public/events`
...
- [ ] 5.1 In `compute_occurrence_start_time`
  ...
  rule changes.
- [ ] 5.2 Test: a weekly evening series spanning a DST boundary keeps a
...
- [ ] 6.1 If query performance needs it, add a denormalized `start_utc`
