...
- [x] 1.1 Add a helper that resolves a `CookieJar` to `Option<(CurrentUser,
...
- [x] 1.2 Every failure mode returns `None`, not an error: no cookie, unparseable
  ...
  works.
- [x] 1.3 The helper must not be added as a router layer. These two routes live on
...
- [x] 2.1 `src/web/templates/event_register.rs`: call the helper before building
...
- [x] 2.2 When a member resolves, build the context with `BaseContext::for_member`
...
- [x] 2.3 Price shown to a signed-in member is `member_price_cents`, rendered `0`
  as "Free" the same way the guest price is. The "Members pay X — log in" line is
  redundant for them and should not render.
- [x] 2.4 `templates/events/register.html`: branch on the signed-in state. Signed
...
- [x] 2.5 The sold-out branch takes precedence over all of the above, as it does
  now.

## 3. Class registration page

- [x] 3.1 `src/web/templates/class_register.rs` and
...
- [x] 3.2 Share the signed-in/registration-state shape between the two pages
...
- [x] 4.1 The "log in to register at the member price" link becomes
  `/login?redirect=<url-encoded current path>`.
- [x] 4.2 The login handlers currently filter the post-authentication destination
...
- [x] 4.3 `login_page`'s already-authenticated branch redirects to
...
- [x] 5.1 Signed-in member opening a paid event's registration page: response
  contains the member price and the authenticated action, and contains no
  `name=` / `email=` input and no Turnstile site key.
- [x] 5.2 Anonymous visitor on the same event: response is unchanged from today —
  guest form present, guest price shown, member price and login link shown.
- [x] 5.3 Request carrying a session cookie that does not validate renders the
...
- [x] 5.4 Signed-in member who already holds a seat sees the already-registered
  panel and no registration action.
- [x] 5.5 Signed-in member requesting a non-publicly-registerable event gets `404`,
  matching the anonymous response for the same id.
- [x] 5.6 The class page repeats 5.1 and 5.4 at series scope.
- [x] 5.7 Login destination allow-list: `/events/<uuid>/register` and
...
- [x] 5.8 Regression guard: assert the anonymous rendering still reaches the guest
