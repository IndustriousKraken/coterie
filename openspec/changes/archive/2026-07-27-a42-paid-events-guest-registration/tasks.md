...
- [x] 1.1 Migration: `ALTER TABLE events ADD COLUMN guest_price_cents INTEGER NOT
  ...
  as a null price.
- [x] 1.2 Same migration: rebuild `event_attendance` with a surrogate
...
- [x] 1.3 Same migration: `CHECK` that exactly one identity is present —
  ...
  from migration 016.
- [x] 1.4 Same migration: `UNIQUE(event_id, member_id)` and
...
- [x] 2.1 `EventAttendance`: `member_id` becomes `Option<Uuid>`, add
...
- [x] 2.2 `Event`: `guest_price_cents: i64` and `guest_registration_enabled: bool`,
...
- [x] 2.3 Repository: seat claim, roster, and refund lookups accept a guest
  identity. The capacity count from a41 is row-based and needs NO change —
  confirm with a test rather than editing it.
- [x] 2.4 `find_event_fee_payment` gains a by-guest-email lookup for the
  double-charge guard.

## 3. Registration service

- [x] 3.1 Guest path on the a41 service: same ordering (claim seat → payment +
  session → release on failure), guest identity on the row and on the payment as
  `Payer::PublicDonor`.
- [x] 3.1b Free guest path (`guest_price_cents == 0`): claim and confirm the seat
...
- [x] 3.2 Guest double-charge guard keyed on `(event_id, guest_email)`.
- [x] 3.3 Do NOT look the guest email up against members. Add a test asserting a
  guest using a member's email produces a guest row and writes nothing into that
  member's account.
- [x] 3.4 Bound and validate guest name/email at the boundary.

## 4. Public surface

- [x] 4.1 `GET /events/:id/register` — public page. 404 unless
  `publicly_registerable()`. Render only public fields; never the roster.
- [x] 4.2 Show member price + login link when a member price exists, so a member
  is not silently charged the guest price.
- [x] 4.3 Sold-out state: say so, render no form.
- [x] 4.4 `POST /public/events/:id/register` — the money endpoint.
- [x] 4.5 Guest confirmation email on seat confirmation: event details, plus a
  receipt only when money was actually paid. Skip silently when no email provider
  is configured.
- [x] 4.6 `PublicEvent` projection gains `registration_url: Option<String>` and
...
- [x] 5.1 Wire `money_limiter` to the new POST, applied BEFORE the bot challenge.
- [x] 5.2 Wire the `BotChallengeVerifier`, fail-closed, matching
  `/public/signup`'s behavior and log outcomes.
- [x] 5.3 Add `POST /public/events/:id/register` to `CSRF_EXEMPT_PATHS` with the
  written justification the exempt-list requirement demands.
- [x] 5.4 Confirm the CORS allowlist covers the endpoint for the marketing origin.
- [x] 5.5 Verify a rate-limited or challenge-failed request claims NO seat and
  creates NO payment row — the rejection must happen before any state is written.

## 6. Admin

- [x] 6.1 Guest price field + "allow non-members to register" checkbox on the
  ...
  do not reject it.
- [x] 6.2 Roster shows guest name/email and visually distinguishes guests from
  members.
- [x] 6.3 At-the-door, comp, release-stuck-seat, and refund all work on guest rows
  and audit identically.

## 7. Tests

- [x] 7.1 A `MembersOnly` event and a nonexistent event id produce
  indistinguishable 404s from the public page.
- [x] 7.2 An event with `guest_registration_enabled = false` is not publicly
...
- [x] 7.2b Free registerable event: page served, seat confirmed with no payment
  row, and `/public/events` emits a `registration_url` with
  `guest_price_cents = 0`. A zero price must not suppress the URL.
- [x] 7.2c A show-up event (guest registration disabled) emits a null
  `registration_url` — the common case for this org's weekly events.
- [x] 7.3 Guest happy path: form → seat held → webhook → `Registered` +
  confirmation email.
- [x] 7.4 Guest abandonment frees the seat (reuses a41's expiry path).
- [x] 7.5 Guest and member seats compete for the same capacity.
- [x] 7.6 Concurrent duplicate guest email yields exactly one seat (exercise the
  UNIQUE constraint, not just the service guard).
- [x] 7.7 Guest using a member's email does not touch that member's account.
- [x] 7.8 Rate-limited request is rejected without consulting the provider and
  without claiming a seat; challenge-failed request likewise writes nothing.
- [x] 7.9 Guest refund releases the seat.
