...
- [x] 1.1 Migration: add a `scheduled_publish_timezone` TEXT column to
...
- [x] 1.2 Carry the zone on the announcement domain/service and add a
...
- [x] 1.3 On create/edit, store the naive `datetime-local` input as-is and
  freeze the zone from `org.timezone`.

## 2. Runner compares the derived instant

- [x] 2.1 In the scheduled-publish query (`announcement_repository.rs:304`)
...
- [x] 3.1 Render the scheduled time in the org zone with its abbreviation
  (e.g. "9:00 AM EDT") on the admin list/detail
  (`src/web/portal/admin/announcements.rs:352`), replacing the "UTC" label.

## 4. Tests (offline)

- [x] 4.1 Non-UTC org: a Draft scheduled for a local time whose true instant
  is still in the future does NOT publish early; it fires on the true instant.
- [x] 4.2 The annotation migration shifts no stored value (rendered local
