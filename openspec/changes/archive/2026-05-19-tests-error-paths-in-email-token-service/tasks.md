...
- [x] 1.1 `consume_redeems_exactly_once` — `create()` then two `consume()`s;
  assert first returns `Some(ConsumedToken { member_id })`, second returns
  `None`.
- [x] 1.2 `consume_rejects_expired_token` — insert a row directly via
...
- [x] 4.1 `created_token_is_sha256_of_plaintext` — `create()` returns
