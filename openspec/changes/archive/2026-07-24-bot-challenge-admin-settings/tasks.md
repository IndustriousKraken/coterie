...
- [x] 1.1 Migration: `INSERT INTO app_settings` for `bot_challenge.provider`
...
- [x] 2.1 Add a `DynamicBotChallengeVerifier` (implements `BotChallengeVerifier`)
...
- [x] 2.2 `main.rs`: construct the dynamic verifier (with `settings_service` +
...
- [x] 3.1 `settings.rs`: mark `bot_challenge.provider` as a select
...
- [x] 4.1 Provider `disabled` in settings → request passes through (no verify).
- [x] 4.2 Provider `turnstile` + stored secret → verify runs; missing token fails
  closed (403), matching the existing bot-challenge scenarios.
- [x] 4.3 The secret round-trips encrypted (stored ciphertext; decrypts for the
  siteverify call) and never appears in logs.
- [x] 4.4 Changing the provider setting changes behavior without reconstructing
  the app (the verifier reads settings live).
- [x] 4.5 Existing bot-challenge suite (fail-closed, disabled opt-out, trait
  swap) still passes.

## 5. Verify

- [x] 5.1 `openspec validate bot-challenge-admin-settings --strict` passes.
- [x] 5.2 `cargo test` (bot-challenge + settings suites) green; `cargo clippy` clean.
