...
- [x] 1.1 `src/domain/event.rs`: add a named constant for the grace period —
...
- [x] 1.2 Add the predicate as a free function over **instants**, not over
...
- [x] 1.3 Add a thin `Event::is_upcoming(&self, now)` that passes
...
- [x] 1.4 Do not add a `has_ended` twin. One predicate, negated where needed.

## 2. Call sites

- [x] 2.1 `src/repository/event_repository.rs::list_upcoming` — replace
...
- [x] 2.2 `src/repository/event_repository.rs::count_members_only_upcoming` —
  same predicate, same widened margin, so the teaser count matches the list it
  teases.
- [x] 2.3 `src/api/handlers/public/mod.rs` — the `None => events.retain(|e|
...
- [x] 2.4 Leave `src/service/series_enrollment_service.rs:141` start-based. It
...
- [x] 2.5 Leave `src/web/templates/class_register.rs:70` start-based for the same
  reason — it counts sessions a buyer would still receive.
- [x] 2.6 `src/web/portal/admin/events/single.rs:142-146` is the admin event
...
- [x] 2.7 Do NOT touch the occurrence cancel/override controls on the series
  ...
  this change decides.
- [x] 2.8 Reminder scheduling is untouched. `event-reminders` canon states a past
  ...
  **Confirmed by inspection.** `list_pending_reminders`
  (`src/repository/event_repository.rs:1082-1165`) is self-contained: it runs
  its own `JOIN` against `event_attendance`, keeps its own 15-hour SQL margin
  (deliberately NOT widened — a reminder window is a start-time window), and
  applies its own exact test `start_utc > now && start_utc <= until` over
  `EventReminderRow::start_utc()`, a different type from `Event`. It calls
  neither `list_upcoming`, `count_members_only_upcoming`, `Event::is_upcoming`,
  nor the free `is_upcoming`, and no reminder path routes through the public
  handler or the admin filter. A past event is still not reminded on its start
  time.

## 3. Tests

- [x] 3.1 `is_upcoming` unit tests at the boundaries: one second before the end
  ...
  day-of-week flakes.
- [x] 3.2 `list_upcoming` includes an event that started an hour ago and ends an
  hour from now, and excludes one that ended a minute ago.
- [x] 3.3 `list_upcoming` sorts the in-progress event ahead of one starting later
  today.
- [x] 3.4 An event with no `end_time` that started 30 minutes ago is included;
  one that started 3 hours ago is not.
- [x] 3.5 `count_members_only_upcoming` returns the same count as the number of
...
- [x] 3.6 `/public/events` with no range includes an in-progress event; the same
...
- [x] 3.7 Double-conversion guard: for an event in a non-UTC zone, assert the
...
- [x] 3.8 `/public/events` still excludes an event that has ended, and the
  `from`/`to` range still returns it.
- [x] 3.9 The admin list's `upcoming` and `past` filters partition the fixture:
  every event appears under exactly one of them, including one in progress.
- [x] 3.10 Assert no call site outside the domain compares a start instant to
