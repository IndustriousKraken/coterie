...
- [x] 1.1 In `src/web/portal/events.rs`, add a small private helper
...
- [x] 1.2 Do NOT add a new visibility predicate to `EventSeries`; reuse
...
- [x] 2.1 In `src/web/portal/events.rs::enroll_in_series`, after the series
  ...
  `&current_user.member`.
- [x] 2.2 When it returns false, return
  ...
  indistinguishable.
- [x] 2.3 Verify by reading the resulting control flow that no
...
- [x] 3.2 In the same test, assert no row was written: `series_enrollments`
  and `payments` for that member are empty, and `event_attendance` has no row
  for the member on any occurrence of the series.
- [x] 3.3 Assert the hidden title does not appear anywhere in the response
  body.
- [x] 3.4 Add `enroll_in_admin_only_series_succeeds_for_an_admin`: the same
  POST from a member with `is_admin` proceeds normally (enrollment held or
  registered, per the class's price).
- [x] 3.5 Add `enroll_in_members_only_series_still_succeeds` so the change
  does not narrow ordinary member enrollment.
- [x] 3.6 Run `cargo test --test admin_only_class_enroll_test`, then `cargo
