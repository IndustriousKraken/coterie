...
- [x] 1.1 In `src/payments/webhook_dispatcher/mod.rs`, under the existing
...
- [x] 1.2 Add `pub async fn dispatch_expired_session(&self, session: CheckoutSession) -> Result<()>`
  forwarding to `self.handle_expired_session(session)`.
- [x] 1.3 Add `pub async fn dispatch_subscription_updated(&self, subscription: stripe::Subscription) -> Result<()>`
  forwarding to `self.handle_subscription_updated(subscription)`.

## 2. Tests for handle_failed_payment

- [x] 2.1 `failed_payment_flips_matching_payment_to_failed` — seed a
...
- [x] 2.2 `failed_payment_for_unknown_id_is_noop` — with no matching payment,
...
- [x] 3.1 `expired_session_flips_pending_payment_to_failed` — seed a
...
- [x] 3.2 `expired_session_for_unknown_session_is_noop` — dispatch a
...
- [x] 4.1 `subscription_updated_refreshes_stored_subscription_id` — seed a
...
- [x] 4.2 `subscription_updated_for_unknown_customer_is_noop` — dispatch a
