...
- [x] 1.1 `src/api/handlers/public.rs` signup: in the payment-mode branch, create
...
- [x] 1.2 Factor the existing `create_signup_checkout` (session contract +
...
- [x] 2.1 `src/web/templates/verify.rs`: after `mark_email_verified`, if the
...
- [x] 3.1 In the abandoned-checkout retry (`retry_pending_checkout`), add the
...
- [x] 4.1 Payment-mode signup (fee>0) → response has no checkout URL and NO Stripe
  customer/session is created.
- [x] 4.2 Verifying a Pending, unpaid, payment-mode, fee>0 member → checkout
  session initiated and presented.
- [x] 4.3 Approval-mode / fee-0 verification → verified + awaiting-review, no
  checkout.
- [x] 4.4 Unverified retry (correct password) → verification re-queued, no session.
- [x] 4.5 Verified retry → resumes open session or refreshes an expired one
  (existing retry behavior preserved).
- [x] 4.6 Completed-payment webhook still activates the Pending member (unchanged).
