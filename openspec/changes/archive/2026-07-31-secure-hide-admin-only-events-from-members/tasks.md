...
- [x] 1.1 In `src/domain/event.rs`, add
  ...
  site.
- [x] 1.2 Add a unit test `admin_only_event_is_hidden_from_non_admin` in
...
- [x] 2.1 In `src/web/portal/events.rs::events_list_api`, extend the
  ...
  the event-type test.
- [x] 2.2 In `src/web/portal/dashboard.rs::upcoming_events`, filter the
...
- [x] 3.1 In `src/web/portal/events.rs::rsvp_event`, after loading the
...
- [x] 4.1 Add a test in `src/web/portal/events.rs`'s test module:
  ...
  titles are present.
- [x] 4.2 Add a test in `src/web/portal/dashboard.rs`'s test module doing
  the same for `/portal/api/events/upcoming`.
- [x] 4.3 Add a test `rsvp_to_admin_only_event_is_refused`: POST
