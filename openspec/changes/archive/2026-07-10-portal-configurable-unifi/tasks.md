...
- [x] 1.1 In `src/service/settings_service.rs`, add a `unifi` key module
...
- [x] 1.2 Add `UpdateUnifiConfig` with the "`Some(nonempty)` = encrypt +
  replace, `None`/blank = keep existing" semantics for the password (mirror
  `UpdateDiscordConfig`).
- [x] 1.3 Migration: delete the dead `integrations.unifi.enabled` row.

## 2. Read config from the store at operation time

- [x] 2.1 Confirm the UniFi integration reads its configuration from the
...
- [x] 3.1 At startup, if the DB has no `unifi.*` values and
...
- [x] 4.1 Add `src/web/portal/admin/unifi.rs` +
  ...
  persisting.
- [x] 4.2 Register `/portal/admin/settings/unifi` (get/post/test) in the
  admin router so it inherits `require_admin_redirect` + CSRF.
- [x] 4.3 Add a "UniFi" link in the admin nav and on the settings index.
- [x] 4.4 Audit-log saves via `audit_service` with the password redacted.

## 5. Tests

- [x] 5.1 Unit: `UpdateUnifiConfig` blank-password-keeps / nonempty-replaces.
- [x] 5.2 Integration: `POST /test` reports success/failure without
  persisting; a save updates the store and the next gating action reads the
  new values.
- [x] 5.3 Seed-from-env: DB empty + env present → seeded once; DB present →
  env ignored.
- [x] 5.4 Assert `/portal/admin/settings` no longer renders the
