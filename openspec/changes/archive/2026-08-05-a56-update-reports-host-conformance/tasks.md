...
- [x] 1.1 `deploy/coterie-provision/src/update.rs`: after the placement and
...
- [x] 1.2 First check: for each unit file the release ships under `deploy/`, is
...
- [x] 1.3 Structure it as a list of checks, each returning an optional finding
  with a message and a resolving command, so adding one later is adding an entry
  rather than editing the reporting logic.
- [x] 1.4 Derive the unit list from what is actually present in the release's
  `deploy/` directory, not from a hardcoded list. A hardcoded list is how a unit
  shipped later goes unchecked.
- [x] 1.5 Route host inspection through the existing `SystemCommand` abstraction.
...
- [x] 1.6 A check that cannot determine the host's state SHALL NOT report a
...
- [x] 2.1 Print each finding with its resolving command. An operator reading the
  output should be able to act without opening a README.
- [x] 2.2 Print nothing — no header, no "all clear" line — when there are no
  findings. The section's presence is the signal, which only works if it stays
  absent otherwise.
- [x] 2.3 Emit through the same `Output` sink the rest of `update` uses.
- [x] 2.4 Exit zero regardless of findings.

## 3. Boundaries

- [x] 3.1 Enable nothing, start nothing, reload nothing. `deployment-updates`
...
- [x] 3.2 Write nothing outside the install directory.
- [x] 3.3 Do not check operator configuration — whether `.env` is complete, or
...
- [x] 4.1 A host with a shipped unit not enabled produces a finding naming the
  unit and the enabling command.
- [x] 4.2 A conformant host produces no output for this at all — assert the
  absence of a header, not just the absence of findings.
- [x] 4.3 Both cases exit zero.
- [x] 4.4 The check issues no `systemctl enable`, `start`, or `reload` — assert
  via the `SystemCommand` fake. This is the guard that keeps 3.1 true as the code
  grows.
- [x] 4.5 A unit added to the release's `deploy/` is checked without a code
  change, proving 1.4.
- [x] 4.6 A failed host query yields no finding rather than a false one.
- [x] 4.7 The full update flow, including this check, still runs under fakes with
  no real process execution.

## 5. Documentation

- [x] 5.1 In `docs/deploy/OPS.md`, note that update reports host discrepancies and
