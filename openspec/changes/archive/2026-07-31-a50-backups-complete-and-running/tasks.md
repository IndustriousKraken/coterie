...
- [x] 1.1 `deploy/backup.sh`: after `VACUUM INTO`, produce a single timestamped
...
- [x] 1.2 Derive the upload roots from the data dir rather than naming them
  absolutely, so a non-default `COTERIE__SERVER__DATA_DIR` still backs up.
- [x] 1.3 Handle a missing or empty upload root as normal, not an error — a fresh
  instance has neither.
- [x] 1.4 Keep retention, weekly/monthly promotion, and the optional S3 push
  operating on the bundle.
- [x] 1.5 Note the growth ceiling in the script: uploads are re-archived in full
...
- [x] 1.6 **Never archive the backup directory into a bundle.** Its default
...
- [x] 1.7 Document that `COTERIE_BACKUP_DIR` should point at a **separate volume**
...
- [x] 2.1 New `deploy/restore.sh` taking a bundle path.
- [x] 2.2 Verify first: the bundle is readable and contains the expected
  components. Fail before touching anything if not.
- [x] 2.3 Then: stop the service; move the existing database **and** both upload
...
- [x] 2.4 Refuse to run as a non-root user with a clear message rather than
  failing halfway through with a permissions error.
- [x] 2.5 Print where the displaced data went, so reversing the restore is
  obvious rather than archaeological.

## 3. Provisioning installs the timer

- [x] 3.1 Wizard install step: copy `coterie-backup.service` and
...
- [x] 3.2 Idempotent — a re-run detects an installed timer and leaves it alone.
- [x] 3.3 Non-interactive equivalent, per the wizard's existing rule that every
  prompt has one.
- [x] 3.4 `--dry-run` shows the backup-install step like any other.

## 4. Documentation

- [x] 4.1 `docs/deploy/RESTORE.md`: restore via the script as the primary path;
  keep the manual steps as the fallback; cover uploads, which it currently does
  not mention at all.
- [x] 4.2 Add the retroactive install for existing deployments — provisioning only
  covers new hosts, and every deployment made before this change has no timer.
- [x] 4.3 `MIGRATION.md`: point at the bundle as the migration artifact rather
...
- [x] 4.4 State in `RESTORE.md` that DO-style disk snapshots are disaster recovery
  and this is operational recovery plus portability — so an operator does not
  conclude one replaces the other.

## 5. Tests

- [x] 5.1 A bundle taken from an instance with attachments and images contains
  all three components.
- [x] 5.2 A truncated bundle is rejected with the service still running and data
  untouched.
- [x] 5.3 Restore preserves displaced data.
- [x] 5.4 Backup and restore both succeed with a non-default data dir — set
...
- [x] 6.1 Stand up an instance against a temp data dir: run migrations, create a
  member, store a submission attachment and an event image so all three bundle
  components are non-empty.
- [x] 6.2 Run `backup.sh` against it and assert a bundle is produced.
- [x] 6.3 Destroy the data dir entirely — database and both upload roots.
- [x] 6.4 Run `restore.sh` with the bundle, then start the instance and assert:
  ...
  *working instance* is.
- [x] 6.5 Tear the temp data dir down so the test leaves nothing behind.
- [x] 6.6 Wire it into the suite so it runs on every change, not once. This is the
