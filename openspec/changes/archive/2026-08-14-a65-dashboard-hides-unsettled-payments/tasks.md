...
- [x] 1.1 `src/web/portal/dashboard.rs::recent_payments` — filter through the
...
- [x] 1.2 Filter **before** `truncate(5)`. The current order truncates first, so
...
- [x] 1.3 Replace the comment that says "regardless of status", which currently
  documents the defect as if it were the intent.

## 2. Prevent the next surface from missing it

- [x] 2.1 Look for any other member-facing surface that lists payments and does
  not route through the predicate. Two were expected and one was missed; the
  question is whether there is a third.
- [x] 2.2 Prefer making the filtered read the easy path — a repository or helper
...
- [x] 2.3 Leave admin surfaces alone. They must keep showing every status; that
  is how an abandoned checkout gets diagnosed.

## 3. Tests

- [x] 3.1 A member with a `Failed` payment sees it on neither the dashboard
  fragment nor the payments list.
- [x] 3.2 The two surfaces agree: for a fixture mixing `Completed`, `Refunded`,
  `Pending`, and `Failed`, the set the dashboard shows is a prefix of what the
  payments list shows.
- [x] 3.3 Ordering guard for 1.2: a member whose five most recent payments are all
...
- [x] 3.4 An admin viewing the same member still sees every status.
- [x] 3.5 Receipts and the dues statement are unchanged.

## 4. Close the loop

- [x] 4.1 The originating report is `IndustriousKraken/coterie#120`, reopened by a
