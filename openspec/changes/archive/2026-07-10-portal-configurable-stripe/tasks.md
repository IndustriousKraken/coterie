...
- [x] 1.1 In `src/service/settings_service.rs`, add a `stripe` key module
  ...
  is not sensitive.
- [x] 1.2 Add `UpdateStripeConfig` with the established "`Some(nonempty)` =
  encrypt + replace, `None`/blank = keep existing" semantics for the two
  secrets (mirror `UpdateDiscordConfig`).
- [x] 1.3 Migration: delete the dead `integrations.stripe.enabled`,
...
- [x] 2.1 Replace the startup-only `Option<Arc<StripeClient>>` with a
...
- [x] 2.2 Add `rebuild_stripe()` that reads current DB config (decrypting
...
- [x] 2.3 Call `rebuild_stripe()` at startup and after every successful
  Stripe settings save.
- [x] 2.4 Point handlers and the webhook dispatcher at the handle (read the
  current value per request) instead of a captured `Arc`.

## 3. `.env` seed-on-first-boot

- [x] 3.1 At startup, if the DB has no `stripe.*` values and
...
- [x] 4.1 Add `src/web/portal/admin/stripe.rs` +
  ...
  persisting.
- [x] 4.2 Register `/portal/admin/settings/stripe` (get/post/test) in the
  admin router so it inherits `require_admin_redirect` + CSRF.
- [x] 4.3 Add a "Stripe" link in the admin nav and on the settings index.
- [x] 4.4 Audit-log saves via `audit_service` with secrets redacted.
- [x] 4.5 If `enabled` is being turned off while any member is
  `billing_mode = stripe_subscription`, require an explicit confirmation
  that names the affected count before applying.

## 5. Tests

- [x] 5.1 Unit: `UpdateStripeConfig` blank-secret-keeps / nonempty-replaces;
  a blank webhook secret resolves to unconfigured.
- [x] 5.2 Integration (using the `FakeStripeGateway` seams): saving a valid
...
- [x] 5.3 Seed-from-env: DB empty + env present → DB seeded once and Stripe
  enabled; DB present → env ignored.
- [x] 5.4 Assert `/portal/admin/settings` no longer renders any
