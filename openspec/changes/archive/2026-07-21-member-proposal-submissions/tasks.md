...
- [x] 1.1 Migration: create the `submissions` table (schema per `design.md`),
  with FKs to `members` and a nullable `event_id` FK to `events`; index
  `submitter_member_id` and `status`.
- [x] 1.2 Register the `submissions.enabled` boolean setting (category
...
- [x] 2.1 `src/domain/submission.rs`: the `Submission` struct and a
  `SubmissionStatus` enum (`submitted`/`under_review`/`accepted`/`declined`/
  `withdrawn`/`scheduled`) with canonical wire strings.
- [x] 2.2 `src/repository/submission_repository.rs`: CRUD + `list_for_member`,
...
- [x] 2.3 `src/service/submission_service/`: create (bounds + open-submission cap
...
- [x] 3.1 `src/web/uploads.rs`: add PDF magic-byte detection (`%PDF-`) and a
...
- [x] 3.2 Add a gated attachment download route (member portal) that loads the
...
- [x] 4.1 Member: `src/web/portal/submissions.rs` — list own, create form,
  edit/withdraw (owner + `submitted`-only for edit). CSRF tokens on all forms.
- [x] 4.2 Admin: `src/web/portal/admin/submissions/` — review queue, detail,
  accept (with optional schedule → promote) / decline, reviewer note. CSRF on
  all decisions; audit each transition.
- [x] 4.3 Templates: render `title`/`abstract`/`reviewer_note` through Askama
  auto-escaping ONLY — no `|safe` on any submission field, in member OR admin
  templates. Keep pages under the existing CSP.
- [x] 4.4 Mount all submission routes behind the `submissions.enabled` check so
  they do not exist when the toggle is off.

## 5. Tests (feature + security regressions)

- [x] 5.1 Create → row persisted with status `submitted` and submitter from the
  session; oversized title/abstract rejected.
- [x] 5.2 **IDOR:** member A reading/editing member B's submission is denied
  (404/403) and discloses nothing; owner succeeds.
- [x] 5.3 **Stored XSS:** a submission titled `<script>…</script>` rendered in
  ...
  site's `main.test.js`.
- [x] 5.4 **Attachment authz:** a non-owner, non-reviewer fetching a private
  attachment is denied; an accepted+public attachment is reachable; the response
  carries `Content-Disposition: attachment`.
- [x] 5.5 **Upload type:** a non-PDF (e.g. a PNG, and a `PK\x03\x04` zip renamed
  `.pdf`) is rejected by the sniff; an oversized PDF is rejected.
- [x] 5.6 **Publication gate:** a member's `public` submission is absent from the
...
- [x] 5.7 **CSRF:** a create/withdraw/decision POST without a valid token is
  rejected. **Cap:** creating past the open-submission cap is refused.
- [x] 5.8 Toggle off → routes not mounted (request returns not-found) and no
  portal entry point.

## 6. Verify

- [x] 6.1 `openspec validate member-proposal-submissions --strict` passes.
- [x] 6.2 `cargo test` (new + existing suites) green; `cargo clippy` clean on
  touched files.
