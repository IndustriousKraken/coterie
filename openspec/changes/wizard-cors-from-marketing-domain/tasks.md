...
- [ ] 1.1 In `deploy/coterie-provision/src/env_template.rs`, add a
...
- [ ] 1.2 Add a rewrite for the `COTERIE__SERVER__CORS_ORIGINS` line
...
- [ ] 1.3 In `deploy/coterie-provision/src/install.rs`, pass the resolved
  marketing domain into `EnvConfig` when rendering `.env`.

## 2. Tests

- [ ] 2.1 Extend the `.env` golden snapshot (or add an `env_template` unit
  ...
  uncommented.
- [ ] 2.2 Add a case asserting a run with NO marketing domain leaves
  `COTERIE__SERVER__CORS_ORIGINS` unset/commented (same-origin default
  preserved).
- [ ] 2.3 Add a case for a marketing domain already prefixed with `www.`
