...
- [ ] 1.1 `src/config/mod.rs`: add `private_uploads_path()` alongside
  ...
  separate rule.
- [ ] 1.2 Verify the deploy docs' backup script covers `{data_dir}` wholesale
...
- [ ] 2.1 `src/web/uploads.rs`: delete `is_submission_attachment` entirely.
- [ ] 2.2 `serve_upload`: keep the path-traversal guard, resolve within the public
  root, and remove the submission lookup.
- [ ] 2.3 Do not replace it with a flag column or a "is this private" query for
  attachments. Attachments are gone from this root; there is no decision to make.

## 2b. Invert the image predicate (fail closed)

- [ ] 2b.1 Replace `is_private_image` with `is_public_image`: match
  `events.image_url` where `visibility = 'Public'`, UNION
  `announcements.image_url` where `is_public = 1`. Positive phrasing only.
- [ ] 2b.2 `serve_upload` serves when the allow-list matches, and otherwise
  requires an authenticated session — so a deleted, cascaded, or not-yet-committed
  row denies instead of publishes.
- [ ] 2b.3 Do NOT move image files. Visibility is mutable; the query returning a
...
- [ ] 2b.4 Confirm the allow-list is exhaustive before shipping: after task 3, the
...
- [ ] 3.1 `save_uploaded_document` call sites for submissions pass the private
  root. The helper already takes a directory parameter, so this is a call-site
  change, not a signature change.
- [ ] 3.2 The gated attachment route resolves within the private root.
- [ ] 3.3 `delete_uploaded_file` call sites for attachments pass the private root.
- [ ] 3.4 Store `attachment_path` with a prefix that distinguishes the root, so a
...
- [ ] 3b.1 `SubmissionService::update_owned` takes an uploads-root parameter,
...
- [ ] 3b.2 Capture the pre-edit path before overwriting `attachment_path`.
- [ ] 3b.3 After the row update succeeds — and only when a new attachment was
...
- [ ] 3b.4 `src/web/portal/submissions.rs::update_submission` passes the root
  through.
- [ ] 3b.5 Document it as reclaiming disk. Do NOT write that the orphan "would
...
- [ ] 3b.6 Test: replacing an attachment leaves exactly one file on disk.

## 4. Migration

- [ ] 4.1 Create the private root at startup if absent, mirroring how the uploads
  directory is created.
- [ ] 4.2 Move each file named by `submissions.attachment_path` from the public
  ...
  still exist.
- [ ] 4.3 Make it idempotent — a re-run with paths already rewritten must be a
  no-op, not an error.
- [ ] 4.4 Leave unreferenced files in the public root untouched. Distinguishing an
...
- [ ] 5.1 An attachment's filename requested at `/uploads/:filename` is not served
  — with its `submissions` row present.
- [ ] 5.2 Same request after the row is deleted — still not served. This is the
  regression that the whole change exists for; assert it explicitly rather than
  trusting the directory split.
- [ ] 5.3 Deleting a member cascades their submissions away and their attachment
  remains unreachable by the public route.
- [ ] 5.4 The gated route still serves the attachment to the owner and to an admin,
  with `Content-Disposition: attachment` and `nosniff` intact.
- [ ] 5.5 A public upload (an event image on a `Public` event) is still served
  normally by `/uploads/:filename`.
- [ ] 5.6 Assert `serve_upload` no longer queries `submissions` — a test that
  fails if someone reintroduces an attachment lookup.
- [ ] 5.8 Deleting a members-only event leaves its image **unreachable**
...
- [ ] 5.9 Flipping an event `Public` → `MembersOnly` → `Public` changes anonymous
  reachability both ways with no file movement.
- [ ] 5.7 Migration idempotency: run it twice, second run is a no-op.
