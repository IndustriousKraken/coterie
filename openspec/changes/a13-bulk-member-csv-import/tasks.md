...
- [ ] 1.1 In `src/service/member_service.rs`, add the typed input/output structs:
  ```rust
  ...
  ```
- [ ] 1.2 Add `MemberService::bulk_import(actor_id: Uuid, rows: Vec<ImportRow>) -> Result<BulkImportSummary>`. Body:
  ...
  - Return the summary.
- [ ] 1.3 The service does NOT do CSV parsing. It takes already-parsed `Vec<ImportRow>` and returns a typed summary.

## 2. Handler — GET form page

- [ ] 2.1 Add `admin_members_import_page` handler in `src/web/portal/admin/members.rs`. Renders `templates/admin/member_import.html` (the upload form with a file input and a format reminder).
- [ ] 2.2 Granular extractors: `State<Arc<CsrfService>>` (for the BaseContext), `Extension<CurrentUser>`, `Extension<SessionInfo>`.

## 3. Handler — POST import submit

- [ ] 3.1 Add `admin_members_import` handler accepting `Multipart`. Granular extractors: `State<Arc<MemberService>>`, `Extension<CurrentUser>`, plus the multipart extractor.
- [ ] 3.2 Body steps:
...
- [ ] 4.1 Create `templates/admin/member_import.html`. Includes:
...
- [ ] 4.2 Create `templates/admin/member_import_result.html` (HTMX fragment). Includes:
...
- [ ] 5.1 In `src/web/portal/mod.rs`, add to the admin sub-router:
...
- [ ] 5.2 The post route inherits the existing CSRF + admin gate. Verify multipart bodies don't trip CSRF (the existing security-middleware spec covers multipart CSRF — see `src/api/middleware/security.rs`).

## 6. UI link on members page

- [ ] 6.1 In `templates/admin/members.html`, add a "Bulk import" link near the existing "New Member" button, pointing at `/portal/admin/members/import`.

## 7. Tests

- [ ] 7.1 Add `tests/admin_member_import_test.rs` (new file). Boot the router with an in-memory pool, seed an admin and an active membership type with slug `regular`.
- [ ] 7.2 Test: happy path. Build a small CSV in-memory with 3 valid rows; POST as multipart; assert response status 200; assert 3 `members` rows exist; assert 3 `import_member` audit rows + 1 `import_members_batch` audit row.
- [ ] 7.3 Test: partial-failure. CSV has 2 valid rows + 1 row with a duplicate email; POST; assert 2 members created, response fragment lists the 1 failure with the duplicate email.
- [ ] 7.4 Test: malformed CSV — missing `email` column in header. POST; assert response is the format-error message; no members created.
- [ ] 7.5 Test: unknown `membership_type_slug` on a row. POST; assert that row is in failures with the unknown slug in the reason.

## 8. Validate

- [ ] 8.1 `cargo build --all-targets --features test-utils` — clean.
- [ ] 8.2 `cargo test --features test-utils` — full suite passes.
