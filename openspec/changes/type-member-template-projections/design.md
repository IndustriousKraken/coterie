...
- **Risk**: rendered date strings drift from the pre-change output (e.g., one template used `%B %d, %Y` and another silently used `%b %d, %Y`; consolidating to one filter changes the output on a page). → **Mitigation**: take the filter format strings literally from each construction site as it exists today. The two formats stay distinct (`fmt_long_date` for dashboard/profile/admin-detail; `fmt_short_date` for admin-members-table). The golden-snapshot test suite (tasks.md Section 8) catches any drift programmatically: goldens are captured from the pre-change templates, then asserted equal after the migration.
