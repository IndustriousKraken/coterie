...
- [ ] 1.1 Confirm `PublicEvent` in `src/api/handlers/public/mod.rs` carries
  ...
  which, do not assume.
- [ ] 1.2 Confirm `description_html` is populated from the **projected**
...
- [ ] 1.3 Confirm the generated OpenAPI schema reflects the field —
...
- [ ] 2.1 Add or extend a test asserting the serialized `/public/events` entry's
...
- [ ] 2.2 Write it so it fails on a field added to the struct **and** on a field
  removed from it. A one-directional assertion catches only half the drift.
- [ ] 2.3 Point the assertion's failure message at this requirement by name, so
  whoever trips it knows the list is canon and not an arbitrary fixture.

## 3. No behavior change

- [ ] 3.1 Change no serialization, no renderer, no template, and no existing
