...
- [ ] 1.1 `deploy/coterie-provision/src/update.rs`: extend the placement step
...
- [ ] 1.2 Copy file-by-file, not by replacing the directory. `static/` and
...
- [ ] 1.3 Before overwriting a file whose contents differ, write the existing
...
- [ ] 1.4 Preserve the executable bit on `*.sh` — `backup.sh` and `restore.sh`
  are invoked directly by the systemd unit and by operators.
- [ ] 1.5 Route every filesystem operation through the existing `FileSystem`
...
- [ ] 2.1 Collect the names of scripts added or changed and emit them through the
  same `Output` sink the rest of `update` uses. Name the files; a bare count tells
  the operator nothing actionable.
- [ ] 2.2 Say nothing when nothing changed. An update that refreshes no script
  should not print a section about scripts.
- [ ] 2.3 Where a `.prev` was written, say so on that file's line, so an operator
  who had pinned a local edit finds out immediately rather than discovering it the
  next time the script runs.

## 3. Boundaries

- [ ] 3.1 Do not touch `/etc/systemd/system` or invoke `systemctl` from `update`.
  ...
  activates units.
- [ ] 3.2 Do not write `.env`. The existing guard stays exactly as it is.
- [ ] 3.3 The idempotent-on-installed-version early return keeps precedence: an
  update that exits because the target equals `VERSION` refreshes nothing,
  including scripts.

## 4. Tests

- [ ] 4.1 With fakes: a release whose `deploy/backup.sh` differs leaves the
  install dir holding the release's version, and a `backup.sh.prev` holding the
  old one.
- [ ] 4.2 A release script byte-identical to the installed one produces no
  `.prev` and no report line.
- [ ] 4.3 A script present in the release but absent from the install dir is
  created.
- [ ] 4.4 A file present in the install dir's `deploy/` but absent from the
  release is left alone — this is the operator-authored case that a wholesale
  directory replace would have deleted.
- [ ] 4.5 The rollback path is unaffected: a failed smoke test restores the
  previous binary, and the test asserts the update did not leave `VERSION`
  claiming the new release.
- [ ] 4.6 Assert no `systemctl` invocation and no write outside the install dir
  occurs during an update, using the existing `SystemCommand` fake. This is the
  guard that keeps 3.1 true as the code changes.
- [ ] 4.7 Executable bits survive the copy for `*.sh`.

## 5. Documentation

- [ ] 5.1 `docs/deploy/OPS.md`: state that `deploy/` now tracks the installed
  release, and that a locally-modified script is preserved as `.prev` when
  replaced.
- [ ] 5.2 Note in the same place that refreshed systemd unit files are not
