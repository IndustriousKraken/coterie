...
- [ ] 1.1 `src/integrations/mod.rs`: add `EventUpdated(Event)`,
  ...
  no purpose.
- [ ] 1.2 Update every consumer match — Discord, UniFi, admin-alert email. None
...
- [ ] 1.3 Do NOT rename `EventPublished`. Its meaning ("created and not
...
- [ ] 2.1 `src/service/event_admin_service.rs::update_one` — currently carries the
...
- [ ] 2.2 Event delete path — dispatch `EventDeleted` after the write.
- [ ] 2.3 Announcement update and delete paths in
...
- [ ] 2.4 Per-occurrence exceptions (cancel, override, restore) in the recurring
  path change what the public feed contains for that occurrence. Dispatch for
  them too.
- [ ] 2.5 Suppress the dispatch when nothing publicly observable changed: an item
  ...
  check per call.
- [ ] 2.6 Reduce the dispatched payload to what the item's own visibility already
...
- [ ] 2.7 Enforce it at dispatch, never by asking consumers to be careful. Sending
...
- [ ] 2.8 Never dispatch on a failed write. Dispatch after the repository call
  returns success, never before or in parallel.

## 3. The notifier

- [ ] 3.1 New settings: a public-site endpoint URL and a shared secret. The
...
- [ ] 3.2 Absent an endpoint, the whole capability is inert: no attempt, no error,
  no log noise. Deployments without a companion site must see zero change.
- [ ] 3.3 Validate the endpoint URL is http(s) on write, the same way
  `org.signup_url` is scheme-checked before it reaches an `href`.
- [ ] 3.4 Sign the request body with the shared secret so the receiver can verify
...
- [ ] 3.5 Payload carries kind, id, and what happened — and **no item content at
  ...
  message.
- [ ] 3.6 Bound every attempt with a short timeout.

## 4. Two delivery paths

- [ ] 4.1 Register the notifier as an `Integration` so it receives creates and
  ordinary updates through the existing fan-out. Loss there is acceptable — the
  companion site's own reconcile catches it.
- [ ] 4.2 Withdrawal — delete, or a visibility/publication change that removes the
...
- [ ] 4.3 Do not weaken `integration-events`' "consumers do not block the
  ...
  that would be wrong.
- [ ] 4.4 A failed withdrawal notification does not roll back the withdrawal.
  The item is withdrawn in Coterie either way; reverting would widen the
  inconsistency, not close it.
- [ ] 4.5 No outbox, no retry queue, no persisted delivery state. The human in the
...
- [ ] 5.1 Per-item resend control on the event detail and announcement detail
...
- [ ] 5.2 Available whether or not the item is currently public — resending a
  withdrawn item's state is precisely how a missed withdrawal gets repaired.
- [ ] 5.3 Hidden entirely when no endpoint is configured.
- [ ] 5.4 The withdrawal response tells the admin plainly whether the public site
...
- [ ] 6.5 A `MembersOnly` item's dispatch carries no more than `/public/events`
  returns for it. Assert against the projection's own output so the two cannot
  drift apart unnoticed.
- [ ] 6.6 No notification payload, for an item of any visibility, contains item
...
- [ ] 6.10 With no endpoint configured: no HTTP attempt on any of the above, and
  every admin action behaves identically to today. Assert the absence of the
  attempt, not just the absence of an error.
- [ ] 6.11 Withdrawal with a reachable endpoint reports success to the admin;
  withdrawal with an unreachable one reports failure, and the item is still
  withdrawn.
- [ ] 6.12 An endpoint that never responds does not hang the admin's request
  beyond the timeout bound.
- [ ] 6.13 The signature verifies against the configured secret, and a body
  altered in transit fails verification.
- [ ] 6.14 The secret does not appear in the settings page HTML or in log output.
- [ ] 6.15 Resend sends for an item that is currently withdrawn.
- [ ] 6.16 The resend control is absent when unconfigured.

## 7. Documentation

- [ ] 7.1 Document the two settings and what a receiver must implement, in the
...
- [ ] 7.2 State plainly that this does not replace the receiver's own
...
- [ ] 7.3 State that withdrawal cannot recall anything a third party already
