...
- [ ] 1.1 In `deploy/coterie-provision/src/main.rs`, add an `Update(Box<UpdateCli>)`
      variant to the `Command` enum with a doc comment ("Update an installed
      Coterie instance to a released version.").
- [ ] 1.2 Define `UpdateCli` with `--tag <vX.Y.Z>` (`Option<String>`, env
...
- [ ] 1.3 Add `pub mod update;` to `deploy/coterie-provision/src/lib.rs`.

## 2. Release resolution (default latest stable, tag override)

- [ ] 2.1 In `src/version_selector.rs`, add a function that, given the parsed
...
- [ ] 2.2 In `update::run`, resolve the target: if `--tag` is set, look it up;
...
- [ ] 3.1 Build the asset + checksum URLs deterministically from the tag
      (`coterie-<tag>-x86_64-unknown-linux-musl.tar.gz` and its `.sha256`),
      matching the names produced by `.github/workflows/release.yml`.
- [ ] 3.2 Download both into a temp dir (behind the existing fetch/`SystemCommand`
...
- [ ] 4.1 Add a snapshot seam (e.g. a `Snapshotter` trait or a method on the
...
- [ ] 4.2 Call the snapshot BEFORE stopping the service or swapping any file. If
      it errors, abort non-zero with context and make no further changes.

## 5. Swap with previous-binary retention

- [ ] 5.1 Stop the service via `SystemCommand` (systemd path; reuse the install
      flow's service-control helper).
- [ ] 5.2 Place the new binaries: install the new `coterie`/`seed` alongside,
...
- [ ] 6.1 Start the service via `SystemCommand`, then call the existing
      `install::smoke_test` (or extract it to a shared location if needed) to
      poll `/health` within `SMOKE_TEST_BUDGET`.
- [ ] 6.2 On smoke-test success, finish and report the installed version.
- [ ] 6.3 On smoke-test failure, restore `coterie.prev`, restart the service,
...
- [ ] 7.1 Before downloading, read the deployment's `VERSION`; if it equals the
      resolved target tag, make no changes and return success (no snapshot, no
      service restart).
- [ ] 7.2 Confirm in code/review that `.env` and the live database file are
      never written by the update path (snapshot output is a separate file).

## 8. Thin bootstrap + script consolidation

- [ ] 8.1 Add `deploy/update.sh` mirroring `deploy/provision.sh`: resolve the
...
- [ ] 8.2 Reduce `deploy/release-deploy.sh` to delegate to `update.sh` (or to
...
- [ ] 9.1 Update the existing `## Update` section in `README.md` (currently
...
- [ ] 10.1 Add unit tests in `version_selector` for: latest-stable selection,
      skipping a prerelease, and exact-tag lookup (hit + miss).
- [ ] 10.2 Add a feature-gated integration test `tests/update_flow.rs`
...
- [ ] 10.3 Add a rollback test: a fake whose smoke test fails causes the previous
      binary to be restored, the service restarted, and a non-zero result with
      the snapshot-guidance message.
- [ ] 10.4 Add tests for idempotency (target == installed `VERSION` → no
      side-effects) and snapshot-failure-aborts-before-any-change.

## 11. Validation

- [ ] 11.1 `cargo build` — clean.
- [ ] 11.2 `cargo test -p coterie-provision --features test-support` — all pass,
      including the new unit and `update_flow` tests.
- [ ] 11.3 `cargo clippy -p coterie-provision --features test-support -- --deny warnings` — clean.
- [ ] 11.4 `cargo fmt --check` — clean.
- [ ] 11.5 `bash -n deploy/update.sh` and `bash -n deploy/release-deploy.sh` — clean.
- [ ] 11.6 `openspec validate a38-coterie-provision-update` — clean.
