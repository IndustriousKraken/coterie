...
- [ ] 1.1 Add `comrak` and `ammonia` to `Cargo.toml`.

## 2. Shared render pipeline

- [ ] 2.1 Add `render_announcement_markdown(md: &str) -> String` (e.g. in
  ...
  stripped.
- [ ] 2.2 Unit tests on the function directly: `<script>` → literal text (not a
...
- [ ] 3.1 `src/web/portal/announcements.rs:132` — replace
...
- [ ] 3.2 Apply the same rendering to any other portal/admin surface that shows
...
- [ ] 4.1 `src/api/handlers/public.rs::list_announcements` — return a response
...
- [ ] 4.2 RSS builder — set each item `description` to the sanitized rendered
  HTML (CDATA-wrapped).
- [ ] 4.3 Tests: `/public/announcements` entry carries a sanitized `content_html`
  (no script/handlers/unsafe schemes); RSS item description carries the rendered
  HTML.

## 5. UI copy

- [ ] 5.1 The "Supports Markdown formatting" hints
  (`announcement_new.html:43,45`, `announcement_detail.html:64`) are now
  accurate — keep them (optionally note the supported subset).

## 6. Verify

- [ ] 6.1 `openspec validate announcement-markdown-rendering --strict` passes.
- [ ] 6.2 `cargo test` green (new + existing announcement/public-feed suites);
