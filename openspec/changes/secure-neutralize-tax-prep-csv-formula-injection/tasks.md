...
- [ ] 2.1 Add an integration test `tax_prep_csv_neutralizes_formula_injection`
...
- [ ] 2.2 In the same test, seed an expense whose category name begins with
  `+` and assert the `category` cell is likewise neutralized.
- [ ] 2.3 Assert in the same test that an ordinary row is unchanged: a
...
- [ ] 2.4 Run `cargo test --test tax_prep_csv_injection_test` and confirm it
