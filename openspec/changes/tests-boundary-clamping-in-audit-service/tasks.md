...
- [ ] 1.1 `prune_older_than_clamps_below_one_day` — seed the table
  ...
  wiped.
- [ ] 1.2 `prune_older_than_clamps_above_3650` — call
...
- [ ] 1.3 `recent_clamps_limit_below_one` — seed the table with three
  rows. Call `audit_service.recent(0)`. Assert `Ok(rows)` is returned
  with `rows.len() == 1` (the lower clamp lifted limit to 1).
- [ ] 1.4 `recent_clamps_limit_above_500` — seed the table with 600
