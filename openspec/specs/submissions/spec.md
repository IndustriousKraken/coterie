...
A member SHALL be able to read, edit, and withdraw ONLY submissions whose
submitter is that same authenticated member, and editing SHALL be permitted only
while the submission is still `submitted`. A request by any member to read or
mutate a submission they do not own SHALL be denied without revealing the
resource, and MUST NOT rely on the id being unguessable. Admins are exempt from
the ownership restriction for review purposes.
...
withdrawing their own submission (per the owner-access requirement), no non-admin
SHALL change a submission's status. On acceptance with a schedule, the service SHALL create a
