...
- [ ] 1.1 Convert `src/web/portal/admin/events.rs` into a module directory
...
- [ ] 1.2 Create `events/single.rs` holding the single-event CRUD handlers and
...
- [ ] 1.3 Create `events/occurrences.rs` holding the per-occurrence
...
- [ ] 1.4 In `events/mod.rs` declare `mod single;` and `mod occurrences;`,
  ...
  (`pub(super)`/`pub`).
- [ ] 1.5 Reconcile `use` imports per submodule (copy the original file's
  `use` block into each, then prune to what each actually references).
- [ ] 1.6 Confirm the handlers still route through `EventAdminService` for
...
- [ ] 2.1 In `deploy/coterie-provision/src/install.rs`, extract the Stripe
...
- [ ] 2.2 Extract the Discord block (~530–578) into `gather_discord_inputs`.
- [ ] 2.3 Extract the UniFi block (~582–620) into `gather_unifi_inputs`.
- [ ] 2.4 Extract the Caddy block (~625–702) into `gather_caddy_inputs`.
- [ ] 2.5 Leave `gather_inputs` as the orchestrator: it still collects the
...
- [ ] 2.6 Preserve exactly, per the `provisioning-wizard` capability: every
...
- [ ] 3.1 In `src/bin/seed.rs`, extract each phase of `main` (current
...
- [ ] 3.2 Keep `main` as the orchestrator: arg parsing, config load, pool
  setup, migrations, the data-clear step, the ordered calls to the phase
  functions, and the final summary `println!` block (~874–end).
- [ ] 3.3 Preserve every `println!` (text, order, and emitted counts) and the
...
- [ ] 4.1 In `src/repository/member_repository.rs`, add a private helper (in
...
- [ ] 4.2 Parameterize the column qualification: `search` uses unqualified
...
- [ ] 4.3 Preserve the per-caller ORDER-BY difference for
...
- [ ] 4.4 Preserve the exact bind order: WHERE binds first — search pattern
...
- [ ] 4.5 Leave the `MemberRepository::search` and
...
- [ ] 5.1 Move the entire `#[cfg(test)] mod tests { … }` block (current lines
...
- [ ] 5.2 Do **not** convert `event_admin_service.rs` into a module directory
...
- [ ] 5.3 Move the test bodies verbatim; do not rewrite assertions. The
...
- [ ] 6.1 `cargo build --features test-utils` — introduces no compiler errors
  beyond the pre-refactor baseline.
- [ ] 6.2 `cargo test --features test-utils` — every test that passed before
...
- [ ] 6.3 `cargo clippy --features test-utils` and `cargo fmt --check` —
  introduce no new warnings or formatting diffs relative to the baseline.
- [ ] 6.4 Spot-check that the split actually relocated code (each named unit
